...
### 1. Authenticate and create the Worker
...
For the first deployment, the simplest bootstrap is to upload the secret and code together from the ignored local secret file:

```bash
npx wrangler deploy --secrets-file .dev.vars
```

Cloudflare stores `LASTFM_API_KEY` as an encrypted Worker secret; the file itself is not uploaded as a static asset. After the Worker exists, ordinary deployments are:
...
### 2. Connect `lastfm.nathanhenderson.ca`
...
- Open the `workers.dev` URL and later `https://lastfm.nathanhenderson.ca`.
- Compare two known public usernames with one timeframe, then the three defaults.
- Confirm totals, Venn counts, ranked artists, and fingerprint render.
- Download a CSV and inspect its headings and rows.
- Copy the share link into a private window; confirm the names/timeframes restore and run.
- Try a nonexistent username and confirm a friendly error appears.
- Open browser developer tools → Network. Confirm the browser calls only your `/api/lastfm/compare` URL and no request or response contains `api_key` or `LASTFM_API_KEY`.
- Request `/api/lastfm/compare` without parameters and confirm a JSON `400` response with no stack trace.
- In the Worker dashboard, confirm Observability records failures without logging usernames or the Last.fm key.
